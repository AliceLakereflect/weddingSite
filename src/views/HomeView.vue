...
<template>
  ...
  <WdHero bg-url="./hero.jpg" :overlay-opacity="0.45">
    <div class="flex flex-col items-center gap-6 text-cream text-center mt-16">
      ...
      <h1 class="font-chenYuluoyan text-8xl md:text-8xl tracking-wider leading-none">
        ...
        <span class="block font-swei text-champagne text-4xl md:text-5xl my-2">&amp;</span>
        {{ config?.brideTitle }}
      </h1>
  ...
  <section class="section-wrapper">
    <div class="max-w-xl mx-auto px-6 flex flex-col items-center gap-6">
      <p class="font-display text-2xl text-charcoal font-light">期待您的蒞臨</p>
      <div class="h-px w-12 bg-gold/50" />
      <p class="font-swei text-base text-wood-400 tracking-wide leading-relaxed whitespace-pre-line">
        請於婚禮前填寫出席回覆
      </p>
      <p class="font-swei text-base text-wood-400 tracking-wide leading-relaxed whitespace-pre-line">
        讓我們為您做好準備
  ...
  <section class="section-wrapper">
    ...
    <div class="mt-8 flex flex-col md:flex-row items-center justify-center gap-4 md:gap-20 text-center">
      <div class="w-72">
        <p class="font-swei text-base text-wood-400 tracking-widest uppercase mb-2">新郎</p>
        <p class="font-sans text-3xl text-charcoal">林昇暉</p>
      </div>
      <div class="text-gold text-2xl font-display">&amp;</div>
      <div class="w-72 pt-8">
        <p class="font-swei text-base text-wood-400 tracking-widest uppercase mb-2">新娘</p>
        <p class="font-sans text-3xl text-charcoal">李淑因</p>
        <p class="font-swei text-lg text-wood-400 mt-1">李進益先生 · 劉少芬女士 之次女</p>
      </div>
    </div>
  </section>
  ...
  <section id="wedding-info" class="section-wrapper">
    ...
    <div class="mt-12 grid grid-cols-2 gap-0 border border-wood-100 divide-x divide-wood-100">
      <WdInfoItem label="日期">
        ...
        <span class="block text-xs text-wood-400 mt-0.5">{{ config?.dateLunar }}</span>
      </WdInfoItem>
    ...
    <div class="mt-8 bg-wood-50 border border-wood-200 rounded-none px-8 py-5 flex flex-col md:flex-row items-center justify-center gap-4 md:gap-12 text-center">
      <div class="flex items-center gap-3 md:ml-8">
        ...
        <span class="font-sans text-base tracking-widest text-wood-600">{{ config?.checkinTime }} 入席</span>
      </div>
      <div class="hidden md:block h-6 w-px bg-wood-200" />
      <div class="flex items-center gap-3">
        <span class="text-gold text-lg">✦</span>
        <span class="font-sans text-base tracking-widest text-wood-600">{{ config?.startTime }} 準時開席</span>
      </div>
    </div>

    <!-- Google Maps CTA -->
    <div class="mt-10 flex flex-col items-center gap-3">
      <p class="font-sans text-sm text-wood-400 tracking-wider">{{ config?.address }}</p>
      <WdButton
        as="a"
        :href="config?.mapsUrl"
        target="_blank"
    ...
    <div v-if="config?.transportation?.length" class="mt-8 border border-wood-100">
      <div class="flex items-center gap-3 px-6 py-4 border-b border-wood-100">
        ...
        <span class="font-sans text-sm text-wood-400 tracking-widest uppercase">交通資訊</span>
      </div>
      <div class="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-wood-100">
        <div v-for="item in config.transportation" :key="item.title" class="px-6 py-5">
          <p class="font-sans text-sm text-gold tracking-wider mb-2">{{ item.title }}</p>
          <p class="font-serif text-base text-charcoal/80 leading-relaxed">{{ item.detail }}</p>
        </div>
      </div>
    </div>
  </section>
